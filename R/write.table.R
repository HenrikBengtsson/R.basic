write.table2 <- function (x, file="", append=FALSE, quote=TRUE, sep=" ", 
  eol="\n", na="NA", dec=".", row.names=TRUE, col.names=TRUE, 
                             qmethod=c("escape", "double"), trim=FALSE) {
  ...
  x <- as.matrix(x);
  if (is.character(x)) {
    # Remove leading and trailing whitespace from the matrix
    if (trim == TRUE) {
      x <- sub("^[ \t\n\f\r ]*", "", x);
      x <- sub("[ \n\f\r ]*$", "", x);
    }
  ...
  for (i in quote) 
    x[, i] <- paste("\"", gsub("\"", qstring, as.character(x[, i])), "\"", sep="");
  writeLines(paste(c(t(x)), c(rep(sep, ncol(x) - 1), eol), 
             sep="", collapse=""), file, sep="");
} # write.table()
